...
export function Dashboard() {
  ...
  return (
    <div>
      <form
        className="mb-6 flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-700 dark:bg-slate-800"
      ...
      >
        <div className="relative min-w-[180px] flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-200 bg-slate-50 py-1.5 pl-8 pr-3 text-sm placeholder:text-slate-400 focus:border-brand-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-100 dark:border-slate-600 dark:bg-slate-700 dark:focus:bg-slate-700 dark:focus:ring-brand-800"
          />
        </div>
        ...
        <select
          ...
          className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-sm focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-100 dark:border-slate-600 dark:bg-slate-700"
        >
        ...
        <select
          ...
          className="rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-sm capitalize focus:border-brand-400 focus:outline-none focus:ring-2 focus:ring-brand-100 dark:border-slate-600 dark:bg-slate-700"
        >
      ...
      <section className="mb-10">
        <div className="mb-4 flex items-baseline justify-between">
          <h1 className="text-xl font-bold tracking-tight">Application Pipeline</h1>
      ...
      {data.reminders_total > 0 && (
        <section>
          <div className="mb-3 flex items-baseline justify-between">
            <h2 className="text-sm font-semibold text-slate-500 dark:text-slate-400">Needs Follow-Up</h2>
            <Link to="/reminders" className="text-xs font-medium text-brand-600 hover:underline dark:text-brand-400">
              View all {data.reminders_total} &rarr;
            </Link>
          </div>
...
function KanbanColumn({
  ...
  return (
    <div className="flex max-h-[75vh] w-72 shrink-0 flex-col rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div className="flex shrink-0 items-center gap-2 border-b border-slate-100 px-3.5 py-3 dark:border-slate-700">
        <span className={`h-2 w-2 rounded-full ${style.dot}`} aria-hidden="true" />
        <span className="text-sm font-semibold capitalize">{status}</span>
        <span className="ml-auto rounded-full bg-slate-100 px-1.5 py-0.5 text-xs font-medium text-slate-500 dark:bg-slate-700 dark:text-slate-400">
          {applications.length}
        </span>
      </div>
      <div
        ref={setNodeRef}
        className={`min-h-16 flex-1 space-y-2 overflow-y-auto p-2.5 transition-colors ${isOver ? 'bg-brand-50 dark:bg-brand-900/40' : ''}`}
      >
        {applications.map((application) => (
          <KanbanCard key={application.id} application={application} onOpen={onOpen} />
        ))}
        {applications.length === 0 && (
          <div className="flex h-16 items-center justify-center rounded-lg border border-dashed border-slate-200 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-500">
            Empty
          </div>
        )}
      </div>
  ...
  return (
    <motion.div layout transition={{ duration: 0.2 }}>
      <button
        ...
        className="block w-full cursor-grab rounded-lg border border-slate-100 bg-white p-3 text-left shadow-sm transition-shadow hover:shadow-md focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-500 active:cursor-grabbing dark:border-slate-700 dark:bg-slate-700/60"
      >
